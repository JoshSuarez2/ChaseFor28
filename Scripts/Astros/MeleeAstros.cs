...
public class MeleeAstros : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(PlayerFound())
        {
            if(Timer >= atkWait && playerHealth.currHealth > 0)
             {
    ...
    private bool PlayerFound()
    {
        ...
        RaycastHit2D hit = Physics2D.BoxCast(box.bounds.center + transform.right * range * transform.localScale.x * dist,new Vector3(box.bounds.size.x * range, box.bounds.size.y, box.bounds.size.z), 0, Vector2.left, 0, playLay);
